...
\begin{itemize}
    ...
    \begin{enumerate}
       \item Bonnes/mauvaises pratiques CAN à plusieurs noeuds
       \item Quoi mettre sur le CAN bus ? 
    \end{enumerate}
\end{itemize}
...
\headerbox{Objectifs semaine prochaine}{name=objectifs_prochain,column=2,span=1,below=risques}{
{
    ...
    \begin{enumerate}
        \item Intégration des équations de dynamique
        \item Intégration des équations de batterie pack
        \item Bien comprendre le solver
        \item Choix de données à afficher
    \end{enumerate}
    \item \textbf{Contrôle :}
    \begin{enumerate}
        \item Documentation et implémentation préliminaire d'une commande vectorielle.
    \end{enumerate}
    \item \textbf{Télémérie :}
    \begin{enumerate}
        \item Estimation de la consommation des composants instrumentation
    \end{enumerate}
...
\headerbox{Cashflow}{name=cashflow,column=0,span=2,below=risques}{
{
    ...
    \vspace{.6cm} % spacing vertical
    {
        \hspace{1cm}
        \centering
    	\includegraphics[scale=.5]{img/jaifaim.png}
    }
